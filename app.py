...
from typing import List, Any  # Pour annotation type

# Chargement du dataset (CSpell peut signaler "Chargement" comme mot inconnu)
df = pd.read_csv("data/insurance.csv")

st.title("Mini-dashboard EDA - Insurance Dataset")

# Filtres interactifs
smoker_filter: List[Any] = st.multiselect(
    "Sélectionner fumeur(s) :",
    options=df['smoker'].unique(),
    default=df['smoker'].unique()
)

region_filter: List[Any] = st.multiselect(
